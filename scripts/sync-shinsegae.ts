...
/*
  변환 규칙은 **공용 모듈에만** 둔다 (scripts/lib/shinsegae-product-map.ts).
  월별 단가표에서 신규 품목만 넣는 add-price-book-products.ts가 같은 규칙을 쓴다 —
  갈라지면 같은 품목이 경로에 따라 다른 unit_normalized·is_food로 들어간다.
*/
import {
  parseSpecQU,
  normalizeTaxType,
...
if (fs.existsSync(envPath)) {
  fs.readFileSync(envPath, 'utf-8').split('\n').forEach((line) => {
    ...
    // ⚠️ 이 저장소의 .env.local은 값을 `"…"`로 감싼다. 벗기지 않으면
    //    supabase-js가 `Invalid supabaseUrl`로 즉시 죽는다 (2026-08-15 확인).
