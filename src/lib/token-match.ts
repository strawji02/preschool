...
export const SUPPLIER_BRANDS: Set<string> = new Set([
  ...
  // CJ
  'cj', 'CJ', '씨제이', '비비고', 'cj이츠웰',
  // (2026-07-04) CJ 대표 브랜드 "백설" — "매실청 백설"에서 브랜드 백설이 키워드가 되어
  //   CJ 백설 제품(대두유·올리고당·부침가루)만 매칭되고 매실청이 밀리던 문제.
  //   "백설기"(떡)는 별개 통짜 토큰이라 영향 없음.
  '백설', '제일제당', 'cj제일제당',
  // 주요 식자재 공급사
  ...
  // 제빵 브랜드 (2026-07-04) — "순수우유후레쉬번 파리바게뜨"의 브랜드가 fallback에서
  // "파리/바게뜨/게뜨" 조각으로 분해돼 식기·바닥솔 노이즈를 끌어오던 문제 방지.
  '파리바게뜨', '파리바게트', '파리크라상', '뚜레쥬르', 'spc삼립', 'cj푸드빌',
])
...
export const GENERIC_MODIFIERS: Set<string> = new Set([
  ...
  // 시대/스타일 (2026-05-12)
  '옛날', '전통', '수제', '가정식', '엄마손맛',
  // 신선도/상태 (2026-05-12) — '생'은 cleanProductQuery에서 3자 이상 식자재명 앞에서만 분리되어 단독 토큰화됨
  '생', '날',
  ...
  '1등급', '2등급', '특등급', '상등급', 'a등급',
  '상품', '특품', '대품', '소품', '중품', '하품',
  // 품위등급 단독 표기 (2026-07-04) — "쑥갓(상)"의 '상'이 '상온/상추'에 오매칭되어
...
export function cleanProductQuery(q: string): string {
  ...
  // (2026-05-12) OCR 노이즈 복원 — '슬라이 스' / '슬라 이스' → '슬라이스'
  // 검수 OCR이 가공 modifier를 공백으로 잘라낸 경우 (예: '생오이피클슬라이 스')
  s = s.replace(/슬라이\s+스/g, '슬라이스')
  s = s.replace(/슬라\s+이스/g, '슬라이스')
  // (2026-05-12) '생' prefix 분리 — "생오이피클슬라이스" → "생 오이피클슬라이스"
  // 3자 이상 식자재명 앞에만 — '생강'(2자)/'생수'(2자)/'생강즙' 같은 단어 보호
  s = s.replace(/(^|\s)생([가-힣]{3,})/g, '$1생 $2')
  // (2026-07-05) 냉동상태 접두 "얼린" 분리 — "한번얼린고등어토막" 통짜 recall 실패 방지.
  //   "한번얼린고등어" → "한번얼린 고등어" (핵심 식자재명 "고등어" 확보)
  s = s.replace(/([가-힣]*)(얼린)([가-힣]{2,})/g, '$1$2 $3')
  ...
  // (2026-07-05) 어류 형태 접미 "토막" 추가 — "고등어토막" → "고등어 토막"
  s = s.replace(/([가-힣]{2,})(슬라이스|커팅|컷팅|필렛|다이스|덩어리|깍둑|채썬|토막)([가-힣]*)/g, '$1 $2 $3')
  // (2026-07-04) 과자 접미 품목어 분리 — 상품 search_vector(mig 047)와 대칭.
  //   검수 "미니쌀약과"가 통짜라 "약과" 검색어가 안 생겨 trigram이 "미니 감자핫도그"·
