...
function item(o: {
  ...
  return {
    ...
    ssg_match: o.matched
      ? { id: o.productId ?? `p-${o.id}`, product_name: o.productName ?? '풀무원 콩나물', standard_price: 800, match_score: 1 }
      : undefined,
    cj_candidates: [],
    ssg_candidates: [],
...
describe('findMatchConflicts — 동일명 상이 확정매칭', () => {
  const items = [
    item({ id: '1', name: '[P]탱탱두부', confirmed: true, matched: true, productId: 'A', productName: '모두부 자연촌' }),
    item({ id: '2', name: '[P]탱탱두부', confirmed: true, matched: true, productId: 'A', productName: '모두부 자연촌' }),
    item({ id: '3', name: '[P]탱탱두부', confirmed: true, matched: true, productId: 'A', productName: '모두부 자연촌' }),
    item({ id: '4', name: '[P]탱탱두부', confirmed: true, matched: true, productId: 'B', productName: '부침용판두부' }),
    item({ id: '5', name: '양파', confirmed: true, matched: true, productId: 'C' }), // 단독 → 충돌 아님
    item({ id: '6', name: '[P]탱탱두부', matched: true, productId: 'B' }), // 미확정 → 제외
  ]

  it('동일명 상이매칭 그룹만 감지 (탱탱두부)', () => {
    const c = findMatchConflicts(items)
    expect(c).toHaveLength(1)
    expect(c[0].displayName).toBe('[P]탱탱두부')
    expect(c[0].totalItems).toBe(4) // 확정 4건 (미확정 #6 제외)
  })

  it('다수결 = 최다 건수 상품 (A ×3), options는 건수 내림차순', () => {
    const c = findMatchConflicts(items)
    expect(c[0].majorityProductId).toBe('A')
    expect(c[0].options[0].productId).toBe('A')
    expect(c[0].options[0].count).toBe(3)
    expect(c[0].options[0].itemIds.sort()).toEqual(['1', '2', '3'])
    expect(c[0].options[1].productId).toBe('B')
    expect(c[0].options[1].count).toBe(1)
  })

  it('상품이 하나로 일치하면 충돌 아님', () => {
    const same = [
      item({ id: '1', name: '무', confirmed: true, matched: true, productId: 'X' }),
      item({ id: '2', name: '무', confirmed: true, matched: true, productId: 'X' }),
    ]
    expect(findMatchConflicts(same)).toEqual([])
  })
})
